...
impl EventLoop {
	...
	async fn handle_event(&mut self, event: SwarmEvent<AsnBehaviourEvent>) {
		match event {
			...
			SwarmEvent::Behaviour(AsnBehaviourEvent::Kademlia(kad::Event::ModeChanged {
				new_mode,
			})) => {
			...
			SwarmEvent::Behaviour(AsnBehaviourEvent::Kademlia(kad::Event::RoutablePeer {
				peer,
				address,
			})) => {
				tracing::info!("Routable peer: {peer} with address {address}");
			},
			SwarmEvent::Behaviour(AsnBehaviourEvent::Kademlia(kad::Event::UnroutablePeer {
				peer,
			})) => {
				tracing::info!("Unroutable peer: {peer}");
			},
			SwarmEvent::Behaviour(AsnBehaviourEvent::Kademlia(kad::Event::RoutingUpdated {
				peer,
				addresses,
				old_peer,
				is_new_peer,
			...
			SwarmEvent::Behaviour(AsnBehaviourEvent::Kademlia(kad::Event::InboundRequest {
			...
			SwarmEvent::Behaviour(AsnBehaviourEvent::Kademlia(event)) => {
			...
			SwarmEvent::Behaviour(AsnBehaviourEvent::Relay(relay::Event::CircuitReqAccepted {
				src_peer_id,
				dst_peer_id,
			})) => {
				tracing::info!("Circuit request accepted from {src_peer_id} to {dst_peer_id}");
			},
			SwarmEvent::Behaviour(AsnBehaviourEvent::Relay(relay::Event::CircuitClosed {
				src_peer_id,
				dst_peer_id,
				error,
			})) => {
			...
			// -- UPnP events
			SwarmEvent::Behaviour(AsnBehaviourEvent::Upnp(upnp::Event::NewExternalAddr(addr))) => {
				tracing::info!("New external address: {addr}");
			},
			SwarmEvent::Behaviour(AsnBehaviourEvent::Upnp(upnp::Event::ExpiredExternalAddr(
				addr,
			))) => {
				tracing::info!("Expired external address: {addr}");
			},
			SwarmEvent::Behaviour(AsnBehaviourEvent::Upnp(upnp::Event::GatewayNotFound)) => {
				tracing::info!("UPnP gateway not found");
			},
			SwarmEvent::Behaviour(AsnBehaviourEvent::Upnp(upnp::Event::NonRoutableGateway)) => {
				tracing::info!("UPnP gateway is not routable");
			},

			// -- auto nat events
			SwarmEvent::Behaviour(AsnBehaviourEvent::AutoNat(autonat::Event::InboundProbe(
			...
			SwarmEvent::Behaviour(AsnBehaviourEvent::AutoNat(autonat::Event::OutboundProbe(
			...
			SwarmEvent::Behaviour(AsnBehaviourEvent::AutoNat(autonat::Event::StatusChanged {
				old,
				new,
			})) => {
			...
			// -- Identify events
			SwarmEvent::Behaviour(AsnBehaviourEvent::Identify(identify::Event::Sent {
				peer_id,
				..
			})) => {
			...
			SwarmEvent::Behaviour(AsnBehaviourEvent::Identify(identify::Event::Pushed {
				peer_id,
				connection_id,
				info: identify::Info { observed_addr, .. },
				..
			...
			SwarmEvent::Behaviour(AsnBehaviourEvent::Identify(identify::Event::Error {
				peer_id,
				connection_id,
				error,
			})) => {
			...
			// -- Rendezvous events
			SwarmEvent::Behaviour(AsnBehaviourEvent::Rendezvous(
			...
			SwarmEvent::Behaviour(AsnBehaviourEvent::Rendezvous(event)) => {
			...
			// -- Gossipsub events
			SwarmEvent::Behaviour(AsnBehaviourEvent::Gossipsub(gossipsub::Event::Message {
			...
			SwarmEvent::Behaviour(AsnBehaviourEvent::Gossipsub(
			...
			SwarmEvent::Behaviour(AsnBehaviourEvent::Gossipsub(gossipsub::Event::SlowPeer {
				peer_id,
				failed_messages,
			})) => {
			...
			// -- Ping events
			SwarmEvent::Behaviour(AsnBehaviourEvent::Ping(ping::Event {
				peer,
				result: Ok(rtt),
				..
			})) => {
